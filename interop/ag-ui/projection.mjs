...
import {createLiveSessionState,projectActivity,snapshotDelta} from './session.mjs';
...
export function projectAgUiRun(run,{observation=null,threadId=null}={}){
  ...
  for(const event of events){
    ...
    if(event.type==='RunStarted'){const snapshot=createStateSnapshot(run,{observation,threadId:resolvedThreadId}),session=snapshot.session;records.push(projected(event,ordinal++,{type:'RUN_STARTED',threadId:resolvedThreadId,runId:run.id,...(run.parentRunId?{parentRunId:run.parentRunId}:{})}));records.push(projected(event,ordinal++,{type:'STATE_SNAPSHOT',snapshot}));records.push(projected(event,ordinal++,{type:'MESSAGES_SNAPSHOT',messages:session.messages}));records.push(projected(event,ordinal++,{type:'ACTIVITY_SNAPSHOT',messageId:`${run.id}:activity`,activityType:'agentsuite.runtime',content:session.activity,replace:true}));protocolStarted=true}
    else if(!protocolStarted)continue;
    else if(event.type==='RunCompleted'){records.push(projected(event,ordinal++,{type:'STATE_SNAPSHOT',snapshot:createStateSnapshot(run,{observation,threadId:resolvedThreadId})}));records.push(projected(event,ordinal++,{type:'RUN_FINISHED',threadId:resolvedThreadId,runId:run.id,outcome:{type:'success'}}))}
    else if(event.type==='RunFailed'){records.push(projected(event,ordinal++,{type:'STATE_SNAPSHOT',snapshot:createStateSnapshot(run,{observation,threadId:resolvedThreadId})}));records.push(projected(event,ordinal++,{type:'RUN_ERROR',message:safeMessage(event),code:safeReason(event)}))}
    ...
    } else if(CUSTOM_NAMES[event.type])records.push(custom(event,CUSTOM_NAMES[event.type],safeFields(payload,['operationId','producedByOperationId','capability','capabilityId','invocationId','origin','sourceId','sourceRef','evidenceRef','focus','sourceKind','safeDisplayName','safeUri','artifactId','type','sourceRunId','roleId','label','reasonCode','code','durationMs','deadline','found','accepted','batch','batches','sources','artifacts','valid','slides','renderer']),ordinal++));
    else records.push(custom(event,'agentsuite.runtime.event',{type:event.type},ordinal++));
    if(protocolStarted&&event.type!=='RunStarted'&&!['RunCompleted','RunFailed','RunCancelled','RunCancellationSettled','RunInterrupted','RunNeedsContext'].includes(event.type)){
      const session=createLiveSessionState(run,{observation:observation||{},threadId:resolvedThreadId}),activity=projectActivity(run),messageId=`${run.id}:activity:${event.sequence}`;
      records.push(projected(event,ordinal++,{type:'STATE_DELTA',delta:snapshotDelta(session)}));
      records.push(projected(event,ordinal++,{type:'ACTIVITY_DELTA',messageId,activityType:'agentsuite.runtime',patch:[{op:'replace',path:'/',value:activity}]}));
      const current=activity.items.find(item=>item.sequence===event.sequence);if(current){records.push(projected(event,ordinal++,{type:'TEXT_MESSAGE_START',messageId,role:'assistant'}));records.push(projected(event,ordinal++,{type:'TEXT_MESSAGE_CONTENT',messageId,delta:current.label}));records.push(projected(event,ordinal++,{type:'TEXT_MESSAGE_END',messageId}));}
    }
  }
