...
export function projectObservation(run,{capabilities=[],configuration={},artifacts=[],contracts=[]}={}){
  ...
  for(const action of actions)terminalRecords.push({kind:'action',sequence:action.firstSequence,channel:action.capability==='MODEL'?'MODEL':['FILES','LOCAL','WEB','MCP'].includes(action.capability)?'SOURCES':action.capability?'ARTIFACTS':'ALL',action});
  ...
  return{runId:run?.id||null,status:run?.status||null,intent,brief:byType.get('Brief')?.data||null,activeStage,currentHarness,role,stages,flowEdges,fullStageEdges,dependencyContractVersion:RELATION_CONTRACT_VERSION,dependencies,agentActions:actions,terminalRecords,safeInputEvents:actions.map(action=>({sequence:action.firstSequence,operationId:action.correlationId,displayInput:action.displayInput,capability:action.capability,stageId:action.stageId})),capabilities:[...caps].map(([id,state])=>({id,state,active:state==='active'})),contextWorld:{roots:[...roots.values()],sources:[...sources.values()],activeSourceId:[...sources.values()].find(item=>item.tracking==='LIVE')?.sourceId||null},evidence:{count:evidence.length,validated:[...validationByEvidence.values()].filter(item=>item.valid).length,unknown:[...validationByEvidence.values()].filter(item=>!item.valid).length,items:evidence},validationDecisions:validation.items||[],claims,outputs,artifacts:artifactRefs,artifactRefs,reusedArtifacts:artifactRefs.filter(item=>item.reused),consoleLines,lastSequence:events.at(-1)?.sequence||0};
}
