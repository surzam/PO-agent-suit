...
try{
  for(const pack of catalog){
    ...
    assert.ok(data.data.rows.length>=8);assert.ok(data.data.insights.length>=3);assert.ok(data.data.researchContext.conflicts.length);assert.ok(story.data.unknowns.some(text=>text.includes(data.data.researchContext.conflicts[0])));assert.ok(story.data.scenes.length>0);assert.ok(story.data.scenes.every(scene=>scene.title.length<90&&scene.humanStatement.text===scene.thesis));assert.ok(story.data.resultClosure.nextAction);
