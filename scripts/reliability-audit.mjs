...
try{
  ...
  assert.deepEqual(full.stages.map(stage=>stage.id),['intent','brief','research','validation','synthesis','data','interactive-result','presentation-story','narrative','hypothesis','slides']);
  ...
  let enteredVerification,releaseVerification;const verificationEntered=new Promise(resolve=>{enteredVerification=resolve}),verificationRelease=new Promise(resolve=>{releaseVerification=resolve});
  const raceHarness={id:'race',inputs:[],outputs:['Presentation'],async execute(){return{artifacts:[{type:'Presentation',data:{html:'race'}}]}}};
  let raceRuntime;raceRuntime=createRuntime({rootDir:temp,registry:createHarnessRegistry([raceHarness]),artifactValidators:{Presentation:async()=>{enteredVerification();await verificationRelease}},runtimeInstanceId:'boot-race'});
  const raceLaunched=await raceRuntime.launch({intent:'cancel race',stages:[{id:'race',harnessId:'race'}],workflowDefinition:{requiredArtifacts:['Presentation']}});await verificationEntered;
  const cancelRace=raceRuntime.cancel(raceLaunched.run.id);await new Promise(resolve=>setTimeout(resolve,5));releaseVerification();await cancelRace;const cancelledRace=await raceLaunched.completion;
  assert.equal(cancelledRace.status,'cancelled','cancellation wins while completion verification is blocked');assert.equal(cancelledRace.events.filter(event=>['RunCompleted','RunFailed','RunCancelled'].includes(event.type)).length,1,'cancel race has one terminal outcome');
  const winningRuntime=createRuntime({rootDir:temp,registry:createHarnessRegistry([raceHarness]),runtimeInstanceId:'boot-race-winner'});const winning=await winningRuntime.run({intent:'completion wins',stages:[{id:'race',harnessId:'race'}],workflowDefinition:{requiredArtifacts:['Presentation']}});assert.equal(winning.status,'completed');assert.equal((await winningRuntime.cancel(winning.id)).status,'completed');assert.equal(winning.events.filter(event=>['RunCompleted','RunFailed','RunCancelled'].includes(event.type)).length,1,'completion winner cannot be rewritten by cancel');
  ...
  const recoveryRuntime=createRuntime({rootDir:temp,registry:createHarnessRegistry([briefHarness]),runtimeInstanceId:'boot-reconcile'});const recoveryRun=await recoveryRuntime.run({intent:'journal recovery',stages:[{id:'brief',harnessId:'brief'}]});const recoveryFile=path.join(temp,'runs',recoveryRun.id,'run.json');const recoverySnapshot=JSON.parse(await fs.readFile(recoveryFile,'utf8'));recoverySnapshot.events.pop();recoverySnapshot.lastAppliedSequence=recoverySnapshot.events.at(-1).sequence;await fs.writeFile(recoveryFile,JSON.stringify(recoverySnapshot));const reconciled=await recoveryRuntime.inspect(recoveryRun.id);assert.equal(reconciled.status,'completed');assert.equal(reconciled.lastAppliedSequence,recoveryRun.events.at(-1).sequence,'journal-ahead snapshot is reconstructed');
  await fs.appendFile(path.join(temp,'runs',recoveryRun.id,'events.jsonl'),'{"partial":');assert.equal((await recoveryRuntime.inspect(recoveryRun.id)).status,'completed','incomplete final JSONL tail is ignored');
  const middleFile=path.join(temp,'runs',recoveryRun.id,'events.jsonl'),middle=await fs.readFile(middleFile,'utf8');const lines=middle.trimEnd().split('\n');lines.splice(1,1,'{"broken":');await fs.writeFile(middleFile,`${lines.join('\n')}\n`);await assert.rejects(()=>recoveryRuntime.inspect(recoveryRun.id),error=>error.code==='JOURNAL_CORRUPT','malformed middle journal record is reported');
  let artifactFailureRuntime=createRuntime({rootDir:temp,registry:createHarnessRegistry([raceHarness]),persistenceHooks:{beforeArtifactRename:async()=>{throw new Error('injected artifact rename failure')}},runtimeInstanceId:'boot-artifact-failure'});const artifactFailure=await artifactFailureRuntime.run({intent:'artifact fault',stages:[{id:'race',harnessId:'race'}]});assert.equal(artifactFailure.status,'failed');assert.equal(artifactFailure.artifacts.length,0);assert.equal(artifactFailure.events.some(event=>event.type==='ArtifactCreated'),false,'failed artifact is not published');
