export class ObservationStore {
  constructor(onChange){this.onChange=onChange;this.run=null;this.projection=null;this.stream=null;this.refreshTimer=null;this.seenOperations=new Set();this.pendingLiveInput=null}
  async attach(id){
    this.stream?.close();clearTimeout(this.refreshTimer);this.pendingLiveInput=null;
    const [run,projection]=await Promise.all([fetch('/api/runs/'+id).then(r=>r.json()),fetch('/api/runs/'+id+'/observation').then(r=>r.json())]);
    this.run=run;this.projection=projection;this.seenOperations=new Set((run.events||[]).map(event=>event.payload?.operationId).filter(Boolean));this.emit();
    this.stream=new EventSource('/api/runs/'+id+'/events?after='+(run.events.at(-1)?.eventId||0));
    this.stream.addEventListener('runtime',event=>{const value=JSON.parse(event.data);if(this.run.events.some(item=>item.eventId===value.eventId))return;this.run.events.push(value);this.run.events.sort((a,b)=>a.sequence-b.sequence);const operationId=value.payload?.operationId,displayInput=value.payload?.displayInput;if(operationId&&displayInput&&!this.seenOperations.has(operationId)){this.seenOperations.add(operationId);this.pendingLiveInput={operationId,sequence:value.sequence,displayInput}}if(value.type==='RunCompleted')this.run.status=value.payload?.status||'completed';if(value.type==='RunFailed')this.run.status='failed';this.scheduleRefresh()})
  }
  ...
  emit(){if(!this.projection)return;const liveInput=this.pendingLiveInput;this.pendingLiveInput=null;this.onChange(this.projection,{liveInput})}
