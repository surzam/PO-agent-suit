...
export function createIntentDiscoveryHarness({ modelJson }) {
  ...
  return {
    ...
    async execute({ context = {}, role, roleDefinition, artifacts, run, observe = async () => {} }) {
      ...
      try {
        await observe('InferenceRequested', { provider:'model', capability:'MODEL', purpose:'intent-discovery', displayInput:'model.infer("intent-discovery")' });
        await observe('InferenceStarted', { provider:'model', capability:'MODEL', purpose:'intent-discovery', displayInput:'model.infer("intent-discovery")' });
        ...
        await observe('InferenceCompleted', { provider: 'model', purpose: 'intent-discovery', status: value?.status || 'discovered' });
        ...
        if (status === 'insufficient-context') return { artifacts: [{ type: 'Intent', data }], events: [{ type: 'IntentDiscoveryInsufficientContext', payload: { requiredContext, reason: data.reason } }], halt: { status: 'needs-context' } };
        return { artifacts: [{ type: 'Intent', data }], events: [{ type: 'IntentDiscovered', payload: { question: data.question } }] };
      } catch (error) {
        await observe('InferenceFailed', { provider: 'model', purpose: 'intent-discovery', code: error.code || 'PROVIDER_ERROR' });
