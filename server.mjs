...
function slidesHtml(plan, meta, data) {
  const style=styleCss[meta.styleId], family=designFamily(meta.styleId), variant=hashSeed(`${meta.styleId}:${meta.generationId}`)%6, metrics=metricRows(data), max=Math.max(...metrics.map(([,value])=>Number(value)||0),1);
  const chart=`<aside class="chart reveal"><div class="chart-title">Data signals</div>${metrics.slice(0,5).map(([name,value,meaning])=>`<div class="bar-row"><label>${esc(name)}</label><div class="bar-track"><i style="width:${Math.max(8,Math.round((Number(value)||0)/max*100))}%"></i></div><b>${esc(value)}</b><small>${esc(meaning)}</small></div>`).join('')}</aside>`;
  const chartIndexes=new Set(); for(let start=0;start<plan.scenes.length;start+=5){const local=plan.scenes.slice(start,start+5).findIndex((scene,offset)=>start+offset>0&&start+offset<plan.scenes.length-1&&['comparison','table','roadmap'].includes(scene.visualType));if(local>=0&&metrics.length)chartIndexes.add(start+local)}
...
.chart{position:absolute;right:92px;top:260px;width:480px;padding:28px;background:var(--ink);color:var(--bg);border:4px solid var(--hot);z-index:2}.chart-title{font:800 16px var(--font-mono);text-transform:uppercase;letter-spacing:.17em;margin-bottom:22px}.bar-row{display:grid;grid-template-columns:145px 1fr 40px;gap:10px;align-items:center;margin:13px 0}.bar-row label,.bar-row b{font:700 13px var(--font-mono);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bar-track{height:13px;background:color-mix(in srgb,var(--bg) 35%,transparent)}.bar-track i{display:block;height:100%;background:var(--hot)}.bar-row small{grid-column:1/-1;font:500 12px var(--font-body);opacity:.7}.has-chart .comparison-composition,.has-chart .table-composition,.has-chart .roadmap-composition{max-width:1080px}.has-chart .roadmap-composition{grid-template-columns:repeat(3,1fr)}.has-chart .road-step:nth-of-type(n+4){display:none}
