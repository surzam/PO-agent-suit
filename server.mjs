...
function templateVariantCss(slug) { const hash=[...String(slug)].reduce((sum,char)=>sum+char.charCodeAt(0),0); const n=hash%16; const tilt=((hash%7)-3)*.35; const titleSize=5.2+(hash%18)/10; const pad=5+(hash%22)/10; const radius=hash%5===0?28:hash%3===0?4:0; const variants=[
...
function slidesHtml(plan, meta, data) {
  ...
  const scenes = plan.scenes.map((s,i)=>`<section class="slide ${i?'':'active'}"><div class="line"></div><div class="meta">${esc(s.visualType)} · ${String(s.index).padStart(2,'0')} / ${plan.scenes.length}</div><h1>${esc(s.title)}</h1><p class="thesis">${esc(s.thesis)}</p><div class="evidence">${s.evidence.map(x=>`<span>${esc(x)}</span>`).join('')}</div><aside class="chart"><div class="chart-title">Data · ${metrics.length} signals</div>${chart}</aside><div class="mark">${i===0?'THESIS':i===plan.scenes.length-1?'NEXT':String(i).padStart(2,'0')}</div></section>`).join('');
