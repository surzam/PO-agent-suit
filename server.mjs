...
async function llama(input, data, temperature) {
  ...
  const base = process.env.LLAMA_BASE_URL || 'http://127.0.0.1:8080/v1'; const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 90000);
  const variation = selfAngles[Date.now() % selfAngles.length];
  try { const response = await fetch(`${base}/chat/completions`, { method:'POST', signal:controller.signal, headers:{'content-type':'application/json'}, body:JSON.stringify({ model:process.env.LLAMA_MODEL || 'Qwen_Qwen3.6-35B-A3B-Q4_K_M.gguf', temperature, messages:[{role:'system',content:PO_SYSTEM_PROMPT},{role:'user',content:`Ракурс этого запуска: ${variation[0]} — ${variation[1]}. Не повторяй прошлую драматургию. Уникальный generationId: ${input.generationId || 'new'}. Запрос: ${input.prompt || 'синтез продукта'}\nData: ${JSON.stringify(data)}`}] }) });
