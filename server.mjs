...
const DEFAULT_PRESENTATION_STYLE_ID = templates.some(template => template.slug === 'blue-professional') ? 'blue-professional' : templates[0]?.slug;
...
// LEGACY-PRODUCTION: the supported /api/generations surface still reaches
// these presentation refinements. They therefore share the canonical local
// provider scheduler with Runtime modelJson calls.
...
async function refineScenes(plan, data, temperature) {
  const base=process.env.LLAMA_BASE_URL || 'http://127.0.0.1:8080/v1';
  const requests=plan.scenes.slice(0,15).map(async (scene,i)=>{ const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),90000); try { const r=await modelScheduler.schedule(()=>fetch(`${base}/chat/completions`,{method:'POST',signal:controller.signal,headers:{'content-type':'application/json'},body:JSON.stringify({model:process.env.LLAMA_MODEL||'Qwen_Qwen3.6-35B-A3B-Q4_K_M.gguf',temperature,messages:[{role:'system',content:'Ты редактор одного слайда Product Owner. Верни только JSON: title, thesis, evidence (1-2 конкретных факта из Data), speakerScript (2-4 живых предложения), visualType. Не повторяй соседние сцены, не придумывай числа.'},{role:'user',content:`Data: ${JSON.stringify(data)}\nВся история: ${plan.centralThesis}\nПредыдущая сцена: ${JSON.stringify(plan.scenes[i-1]||null)}\nТекущая сцена: ${JSON.stringify(scene)}\nСледующая сцена: ${JSON.stringify(plan.scenes[i+1]||null)}`}]})}),{signal:controller.signal}); if(!r.ok)return scene; const raw=(await r.json()).choices?.[0]?.message?.content||''; const m=raw.match(/\{[\s\S]*\}/); return m?{...scene,...JSON.parse(m[0]),index:i+1}:scene; }catch{return scene}finally{clearTimeout(timer)} });
...
function resolvePresentationStyle(requestedStyleId = null) {
  const requested=typeof requestedStyleId === 'string' && requestedStyleId.trim() ? requestedStyleId.trim() : null;
  const known=new Set(templates.map(template=>template.slug));
  const styleId=known.has(requested) ? requested : DEFAULT_PRESENTATION_STYLE_ID;
  if(!styleId)throw new Error('Presentation template library is unavailable');
  return { requestedStyleId:requested, styleId, layoutFamily:designFamily(styleId), fallback:Boolean(requested&&!known.has(requested)) };
}
...
.slide.active,.slide.visible{visibility:visible;opacity:1;pointer-events:auto;z-index:1;transform:none}.deck-controls{position:fixed;left:50%;bottom:18px;transform:translateX(-50%);z-index:1000;display:flex;align-items:center;gap:12px;color:#fff;font:11px var(--font-mono);letter-spacing:.14em;background:#000b;padding:7px 9px;border-radius:99px}.deck-controls button{width:34px;height:30px;border:1px solid #ffffff55;border-radius:99px;background:#ffffff12;color:#fff;font:700 16px var(--font-mono);cursor:pointer}.deck-controls button:hover{background:#fff;color:#050505}.deck-position{min-width:58px;text-align:center}
.scene-header{display:flex;justify-content:space-between;border-bottom:2px solid var(--soft);padding-bottom:17px;font:700 16px var(--font-mono);letter-spacing:.17em;text-transform:uppercase;color:var(--accent)}.scene-header em{font-style:normal}.scene-title{font:800 clamp(56px,4.4vw,84px)/.96 var(--font-display);letter-spacing:-.045em;margin:72px 0 48px;max-width:1400px;overflow-wrap:anywhere;text-wrap:balance}.lead{font:600 36px/1.22 var(--font-body);max-width:1050px}
...
.title-composition{position:absolute;left:92px;right:92px;top:205px;bottom:155px;display:grid;grid-template-columns:320px 1fr;grid-template-rows:1fr auto;align-items:center}.title-index{grid-row:1/3;font:800 280px/.75 var(--font-display);color:var(--accent);opacity:.26}.title-composition h1{font:800 clamp(70px,6.7vw,128px)/.9 var(--font-display);letter-spacing:-.065em;margin:0;max-width:1250px;overflow-wrap:anywhere;text-wrap:balance}.title-composition .lead{grid-column:2}
.closing-composition{position:absolute;inset:205px 170px 160px;display:flex;flex-direction:column;justify-content:center}.closing-kicker{font:700 20px var(--font-mono);text-transform:uppercase;letter-spacing:.2em;color:var(--accent)}.closing-composition h1{font:800 clamp(70px,6.6vw,126px)/.92 var(--font-display);letter-spacing:-.06em;margin:24px 0 34px;max-width:1450px;overflow-wrap:anywhere;text-wrap:balance}.closing-line{width:260px;height:18px;background:var(--hot);margin-top:50px}
...
document.getElementById('deckPrev').addEventListener('click',()=>go(current-1));document.getElementById('deckNext').addEventListener('click',()=>go(current+1));
