...
function fallbackPlan(input, data) {
  ...
  scenes.forEach((scene, i) => { const row=data.rows[i % data.rows.length]?.join(' · ') || 'нет строки Data'; scene.thesis = i === 0 ? `${set.thesis} Ракурс: ${angle[0]}.` : `${scene.title} ${purposes[i]}.`; scene.evidence=[`Data: ${row}`, data.insights[(i*2+seed)%data.insights.length] || 'Источник требует проверки.', `Код: ${scene.codeExample}`]; scene.speakerScript=i===0 ? `Я начинаю с ракурса «${angle[0]}» и вопроса «${prompt}». ${set.thesis} Проверим эту мысль по строкам Data, а не по обещаниям.` : `${transitions[(i+seed)%transitions.length]} Здесь мы ${purposes[i]}. В Data это видно так: ${row}. Следующий слайд переводит наблюдение в решение.`; });
  return { topic: `${promptTitle(prompt)} · ${angle[0]}`, audience:'Product Owner, команда и сам Suite как предмет исследования', centralThesis:`${set.thesis} Ракурс этого запуска: ${angle[0]}.`, motto:`${set.thesis} Опора доклада — ${data.insights[0] || 'проверяемый сигнал из Data'}.`, situation:`Каждый запуск исследует продукт через новую грань — ${angle[1]}. Входной запрос: ${prompt}.`, evidence:data.insights.slice(0, 3), unknowns:['Эффект выбранного ракурса ещё не измерен в рабочем контуре.'], nextStep:`Повторить генерацию и сравнить новую грань «${angle[0]}» с предыдущей.`, angle:angle[0], scenes };
}

const PO_SYSTEM_PROMPT = `Ты старший Product Owner. Отделяй цель, проблему, факт, интерпретацию, ограничение и следующий шаг. Не придумывай числа. Не пересказывай служебные инструкции, AGENTS.md, skills, тесты или код. Используй переданный Data и codeSignals: показывай реальные модули, маршруты и fixture-метрики, но не выдавай гипотезу за факт. На запрос и контекст верни только JSON StoryPlan: topic, audience, centralThesis, situation, evidence (массив), unknowns (массив), nextStep, scenes (10-15 объектов с index,title,thesis,evidence,speakerScript,visualType). Каждый title, thesis, evidence и speakerScript должен добавлять новую мысль и отличаться от соседних сцен; не повторяй один универсальный текст. Один тезис на сцену; заголовок — мысль выступающего; speakerScript — 2-5 живых предложений. visualType: statement|comparison|table|flow|quote|roadmap. Русский язык.`;
async function llama(input, data, temperature) {
  input = { ...input, prompt: input.prompt || selfPrompts[Date.now() % selfPrompts.length] };
  const base = process.env.LLAMA_BASE_URL || 'http://127.0.0.1:8080/v1'; const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 3500);
  const variation = selfAngles[Date.now() % selfAngles.length];
  try { const response = await fetch(`${base}/chat/completions`, { method:'POST', signal:controller.signal, headers:{'content-type':'application/json'}, body:JSON.stringify({ model:process.env.LLAMA_MODEL || 'local', temperature, messages:[{role:'system',content:PO_SYSTEM_PROMPT},{role:'user',content:`Ракурс этого запуска: ${variation[0]} — ${variation[1]}. Не повторяй прошлую драматургию. Запрос: ${input.prompt || 'синтез продукта'}\nData: ${JSON.stringify(data)}`}] }) });
    if (!response.ok) return null; const raw = (await response.json()).choices?.[0]?.message?.content || ''; const match = raw.match(/\{[\s\S]*\}/); const parsed=match ? JSON.parse(match[0]) : null; return parsed ? refineScenes(parsed,data,temperature) : null;
